...
void sendJWS() {
  ...
  char outt[12];
  sprintf(outt, ";%d:%d:%02d", rBul, rTah, rTgl);
  pesan+=String(outt);
...
void handleAtur(String x) {
  File file = LittleFS.open(fileAtur, "r");
...
//   dInfo[b]=(c=="true");
   uint16_t buff = p_info.enable_info;
   bitWrite(buff, b, (c=="true"));
   change_enable(LittleFS, fileInfo, buff);
}
